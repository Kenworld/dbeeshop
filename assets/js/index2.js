...
class RecentlyViewedManager {
  ...
  async displayRecentlyViewed() {
      ...
      try {
          ...
          const recentlyViewedHTML = validProducts.map(product => `
              <div class="col-lg-3 col-md-4 col-sm-6 mt-4">
                  <div class="product-item">
                      <div class="product-img">
                          <a href="single-product.html?code=${product.productCode}">
                      ...
                      <div class="product-content">
                          ...
                          <div class="price-box">
                              <span class="regular-price">GH₵ ${utils.formatTotal(product.salePrice || product.regularPrice)}</span>
                          </div>
                      </div>
                  </div>
              </div>
...
class CartManager {
  ...
  clearCart() {
   ...
   getItemQuantity(productId) {
    ...
    addItem(product) {
      ...
      const existingProductIndex = this.cart.findIndex(item => item.prod_id === productId || item.code === productId);
  ...
  updateQuantity(productId, action) {
    ...
    const productIndex = this.cart.findIndex(item => 
        item.prod_id === productId || item.code === productId
    );
    ...
    if (action === "increase") {
        ...
        if (this.cart[productIndex].quantity > 1) {
            ...
            // Handle removal
            const productContainer = $(`.decreaseBtn[data-dic="${productId}"]`).closest('.d-flex');
    ...
    generateCartPageItemHTML(item, index) {
      ...
      <div class="cart-item mb-4 pb-4 border-bottom" data-index="${index}">
          <div class="row align-items-center">
              ...
              <div class="col-md-2">
                  <span class="price fs-6">GH₵ ${utils.formatTotal(item.price)}</span>
              </div>
              <div class="col-md-2">
                  <div class="d-flex align-items-center justify-content-center">
                      <a class="btn btn-sm btn-outline-secondary decreaseBtn" data-dic="${item.prod_id || item.code}">-</a>
                      <span class="mx-2 itemQty-${item.prod_id || item.code}">${item.quantity}</span>
                      <a class="btn btn-sm btn-outline-secondary increaseBtn" data-dic="${item.prod_id || item.code}">+</a>
                  </div>
              </div>
              <div class="col-md-2">
    ...
    setupCartPageListeners() {
      ...
      $(".decreaseBtn, .increaseBtn").click((e) => {
          ...
          const action = $(e.currentTarget).hasClass("decreaseBtn") ? "decrease" : "increase";
...
displayOrderSummary() {
  ...
  const deliveryFee = 20.00; // You can make this dynamic based on location
...
setupCheckoutListeners() {
  ...
  $("#CheckoutBtn").on("click", async (e) => {
      ...
      try {
          ...
          utils.showToast('errorToast', 'Failed to process order. Please try again.');
  ...
  setupLogoutListener() {
    ...
    if (logoutBtn) {
        logoutBtn.addEventListener('click', async (e) => {
            ...
            const $button = $(e.currentTarget);
            $button.prop('disabled', true);
            $button.html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Logging out...');
...
async logout() {
        ...
        console.log('Loading orders for user:', this.currentUser.uid); // Debug log
          ...
          querySnapshot.forEach((doc) => {
              ...
              order.items.forEach((item, index) => {
                  const date = order.orderDate ? new Date(order.orderDate.toDate()).toLocaleDateString() : 'N/A';
                  ...
                  const row = `
                      <tr>
                          ...
                          <td>GH₵ ${utils.formatTotal(item.price * item.quantity)}</td>
                          <td>
  ...
  setupOrderViewListeners() {
      $('.view-order').off('click').on('click', async (e) => {
        ...
        const trackingHtml = this.generateTrackingTimeline(order.status);
          ...
          let itemsHtml = order.items.map(item => `
              <div class="d-flex justify-content-between mb-2">
                  ...
                  <div>
                      <span>${item.quantity} x GH₵ ${utils.formatTotal(item.price)}</span>
                  </div>
              </div>
          `).join('');
          ...
          const modalContent = `
              <div class="modal-body">
                ...
                <div class="d-flex justify-content-between fw-bold mt-2">
                    <span>Total:</span>
                    <span>GH₵ ${utils.formatTotal(order.total + order.deliveryFee)}</span>
                </div>
  ...
  setupReorderListener(order) {
    $('.reorder-btn').off('click').on('click', async (e) => {
        e.preventDefault();
        const $button = $(e.currentTarget);
...
setupInvoiceListener(order, orderId) {
  $('.download-invoice-btn').off('click').on('click', async (e) => {
      e.preventDefault();
      const $button = $(e.currentTarget);
...
async generateInvoice(order, orderId) {
  try {
      ...
      const invoiceDetails = [
          `Invoice No: ${orderId}`,
          `Date: ${order.orderDate ? new Date(order.orderDate.toDate()).toLocaleDateString() : 'N/A'}`,
          `Status: ${order.status}`,
          '\nBilling Address:',
      ...
      const summaryText = [
          ...
          `Total Amount: GH₵ ${utils.formatTotal(order.total + order.deliveryFee)}`
      ];
        ...
        if (user) {
            ...
            if (window.location.pathname.includes('single-product.html')) {
                ...
                if (productCode) {
                    ...
                    this.recentlyViewedManager.addToRecentlyViewed(productCode)
                        .then(() => {
...
async getProductByCode(productCode) {
  try {
      const q = query(this.productsRef, where("productCode", "==", productCode));
 ...
 async loadProducts() {
    try {
      ...
      $("#productList").html('<div class="alert alert-danger">Failed to load products. Please try again later.</div>');
    }
  }
  async loadBrandProducts(brandName) {
    try {
      ...
      if (result.total < 1) {
        $("#allProductsList").html('<div class="alert alert-danger">No products found for this brand.</div>');
        return;
      }
      this.renderProducts(result.products);
    } catch (err) {
      console.error("Error loading brand products:", err);
      $("#allProductsList").html('<div class="alert alert-danger">Error loading products. Please try again later.</div>');
    }
  }
  async loadCategoryProducts(category) {
    try {
        ...
        if (result.total < 1) {
            $("#allProductsList").html('<div class="alert alert-danger">No products found in this category.</div>');
            return;
        }
        this.renderProducts(result.products);
    } catch (err) {
...
async loadProducts() {
  try {
      ...
      console.error("Error loading products:", err);
      $("#allProductsList").html('<div class="alert alert-danger">Failed to load products. Please try again later.</div>');
  }
}
renderProducts(products) {
  ...
  products.forEach(product => {
      ...
      const html = this.productManager.generateProductHTML(product, productInCart);
  ...
  displayProductDetails(product) {
    ...
    metaDescription.content = product.metaDescription || product.shortDescription || product.name;
    ...
    // Update price
    $('.regular-price').text(`${utils.formatTotal(product.salePrice || product.regularPrice)}`);
    
    // Update descriptions
    $('.shortDesc').html(product.shortDescription || 'No short description available');
    ...
    generateProductHTML(product, productInCart = null) {
      ...
      return `
          <div class="col-lg-4 col-md-4 col-sm-6 product">
              <div class="product-inner card">
                  <div class="thumb">
                      <a href="${productUrl}" class="image product-link" data-product-code="${product.productCode}">
                          <img class="first-image product-img" src="${product.main_image}" alt="${product.name}" />
                          <img class="second-image fit-image" src="${product.main_image}" alt="${product.name}" />
                      </a>
                      ${product.salePrice ? `<span class="badges"><span class="sale">-${percentageOff.toFixed(0)}%</span></span>` : ""}
                  </div>
                  <div class="content">
                      <h5 class="title">
                          <a href="${productUrl}" class="product-link" data-product-code="${product.productCode}">${product.name}</a>
                      </h5>
                      <div class="d-flex align-items-center">
                          <h5 class="new mb-0">GH₵ ${product.salePrice || product.regularPrice}</h5>
                          ${product.salePrice ? `<span class="old ms-2 text-muted">GH₵ ${product.regularPrice}</span>` : ""}
...
class AuthManager {
  ...
  async signup(email, password, userData) {
    try {
      ...
      await this.saveUserDetails(userCredential.user.uid, { ...userData, email });
      window.location.href = "index.html";
    } catch (error) {
    ...
    return order.items.map(item => `
      <tr>
        <td class="align-middle border-top-0 w-0">
          <a href="#"><img width="50" height=50 src="${item.imageUrl}" alt="${item.title}" class="icon-shape icon-xl" /></a>
        </td>
        ...
        <td class="align-middle border-top-0">${order.createdAt.toDate().toDateString()}</td>
        ...
        <td class="align-middle border-top-0">GH₵ ${utils.formatTotal(item.price * item.quantity)}</td>
...
Object.assign(ShopApp.prototype, {
  ...
  async setupAccountPage() {
    ...
    try {
      ...
      const ordersHTML = orders.map(order => this.orderManager.generateOrderHTML(order)).join('');
