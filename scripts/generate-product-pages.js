...
async function generateProductPages() {
    try {
        const template = await fs.readFile('templates/product-template.html', 'utf-8');
        ...
        for (const doc of productsSnapshot.docs) {
            ...
            const galleryHTML = product.other_images?.map(img => `
            ...
            const thumbsHTML = product.other_images?.map(img => `
                <div class="swiper-slide">
                    <img src="${img}" alt="${product.name}">
                </div>
            `).join('') || '';
            ...
            let productHTML = template
                ...
                .replace(/{{META_DESCRIPTION}}/g, product.metaDescription || product.shortDescription)
                .replace(/{{PRODUCT_NAME}}/g, product.name)
                .replace(/{{PRODUCT_IMAGE}}/g, product.main_image)
                .replace(/{{PRODUCT_CODE}}/g, product.productCode)
                .replace(/{{CANONICAL_URL}}/g, `https://yoursite.com/products/${fileName}`)
                .replace(/{{PRODUCT_PRICE}}/g, product.salePrice || product.regularPrice)
                .replace(/{{SHORT_DESCRIPTION}}/g, product.shortDescription || '')
                .replace(/{{PRODUCT_DESCRIPTION}}/g, product.description || '')
                .replace(/{{AVAILABILITY}}/g, product.isAvailable ? 'In Stock' : 'Out of Stock')
...
async function generateProductIndex(products) {
    try {
        ...
        const indexTemplate = await fs.readFile('templates/products-index.html', 'utf-8');
        console.log('Loaded index template');
        
        const productGrid = products.map(doc => {
            const product = doc.data();
                ...
                <div class="col-lg-4 col-md-6 col-sm-6 mb-8">
                    <div class="product-item">
                        ...
                        <div class="product-content">
                            <h5 class="title">
                                <a href="/products/${fileName}">${product.name}</a>
                            </h5>
                            <div class="price-box">
                                <span class="regular-price">GH₵ ${product.salePrice || product.regularPrice}</span>
                            </div>
                        </div>
                    </div>
                </div>
